...
AsyncFileLoad::AsyncFileLoad(const char* const _pWaveName, WaveDataID name)
	: waveID(name), pWaveName(_pWaveName)
{
}

void AsyncFileLoad::execute()
{
	...
	Command* pCmd = new AsyncFileCompletedCommand(this->waveID, pWfx, rawBuffSize, pRawBuff);
	assert(pCmd);

	pOut->PushBack(pCmd);
}

void AsyncFileLoad::privLoadBuffer(const char* const _pWaveName, WAVEFORMATEXTENSIBLE*& pWfx, RawData*& pRawBuff, unsigned int RawBuffSize)
{
	assert(_pWaveName);
	assert(pRawBuff);
	assert(pWfx);
...
unsigned int AsyncFileLoad::privGetBuffSize(const char* const _pWaveName)
{
	assert(_pWaveName);
